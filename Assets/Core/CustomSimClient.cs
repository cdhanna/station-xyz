...
namespace Beamable.Experimental.Api.Sim
{
	...
	public class CustomSimClient
	{
		...
		public delegate void CustomEventCallback<T>(T body);
		private Dictionary<string, List<CustomEventCallback<string>>> _eventCallbacks = new Dictionary<string, List<CustomEventCallback<string>>>();
		private List<CustomEventCallback<string>> _curCallbacks = new List<CustomEventCallback<string>>();
		...
		public CustomSimClient(SimNetworkInterface network, long framesPerSecond, long targetNetworkLead)
		{
		...
		public CustomEventCallback<string> On<T>(string evt, string origin, CustomEventCallback<T> callback)
		{
		...
		public CustomEventCallback<string> OnInternal(string evt, string origin, CustomEventCallback<string> callback)
		{
			List<CustomEventCallback<string>> callbacks;
			if (!_eventCallbacks.TryGetValue(evt + ":" + origin, out callbacks))
			{
				callbacks = new List<CustomEventCallback<string>>();
		...
		public void Remove(CustomEventCallback<string> callback)
		{
			foreach (KeyValuePair<string, List<CustomEventCallback<string>>> entry in _eventCallbacks)
			{
		...
		public CustomEventCallback<string> OnInit(CustomEventCallback<string> callback) { return OnInternal("init", "$system", callback); }
		...
		/// <returns>An instance that can be sent to the <see cref="Remove"/> method to remove the handler.</returns>
		public CustomEventCallback<string> OnConnect(CustomEventCallback<string> callback) { return OnInternal("connect", "$system", callback); }
		...
		/// <returns>An instance that can be sent to the <see cref="Remove"/> method to remove the handler.</returns>
		public CustomEventCallback<string> OnDisconnect(CustomEventCallback<string> callback) { return OnInternal("disconnect", "$system", callback); }
		...
		/// <returns>An instance that can be sent to the <see cref="Remove"/> method to remove the handler.</returns>
		public CustomEventCallback<string> OnTick(CustomEventCallback<long> callback)
		{
		...
		public delegate void CustomHandler(string evt, string origin, string body);

		private List<CustomHandler> _handlers = new List<CustomHandler>();
		public CustomHandler OnMessage(CustomHandler handler)
		{
			_handlers.Add(handler);
			return handler;
		}

		public void RemoveMessageHandler(CustomHandler handler)
		{
		...
		private void TriggerEventCallbacks(string evt, string origin, string body)
		{
			...
			var clone = new HashSet<CustomHandler>(_handlers);
			foreach (var handler in clone)
			{
				handler?.Invoke(evt, origin, body);
			}
			...
			{
				CustomEventCallback<string> next = _curCallbacks[i];
